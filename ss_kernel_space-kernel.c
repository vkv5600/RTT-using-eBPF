...
{   __u64 ts;
    ...
    if(data < data_end)
    {
        ...
        if (iph->protocol == 0x6) 
        {
            ...
            bpf_printk("src_port=%d, dest_dport=%d, eACK=%d", bpf_htons(tcph->source), bpf_htons(tcph->dest), bpf_htons(tcph->ack_seq));
    ...
    if(data < data_end)
    {
        ...
        if (iph->protocol == 0x6) 
        {
            ...
            if (data + sizeof(*eth) + sizeof(*iph) + sizeof(*tcph) > data_end)
             ...
             bpf_printk("\nSecond packet") ;    
            bpf_printk("ip_src=%x, ip_dst=%x,", bpf_htonl(iph->saddr), bpf_htonl(iph->daddr));    
            bpf_printk("src_port=%d, dest_dport=%d, sequence_num=%d", bpf_htons(tcph->source), bpf_htons(tcph->dest), bpf_htons(tcph->seq));
            __u64 flowid=bpf_htonl(iph->saddr)+bpf_htonl(iph->daddr)+bpf_htons(tcph->source)+bpf_htons(tcph->dest)+bpf_htons(tcph->ack_seq);
            bpf_printk("Flow ID: %x",flowid);
            rec = bpf_map_lookup_elem(&counter_map, &flowid);
 	    if(rec == NULL)
            {	bpf_printk("unable to find the packet with the given flowid in the table");
            }
            else
           {	bpf_printk("Found a match.............................................................");
           	rTT=ts-rec->rx_packets;
           	bpf_printk("RTT: %llu\n",rTT);
           	bpf_map_update_elem(&RTT,&flowid,&rTT,1);
           	bpf_map_delete_elem(&counter_map, &flowid);
           }
        }
        else if (iph->protocol == 0x11)
        {
