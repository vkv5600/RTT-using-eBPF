...
char tc_map_name2[] = "counter_map1";
char tc_map_name3[] = "timestamp";
char tc_map_name4[] = "RTT";
...
void main ()
{
    struct datarec rec1,rec2,rec3,rec4;
    ...
    map_fd3 = open_bpf_map_file(tc_pin_base_dir, tc_map_name3, NULL);
    map_fd4 = open_bpf_map_file(tc_pin_base_dir, tc_map_name4, NULL);
    if (map_fd1 < 0 || map_fd2<0) {
        printf("finding the map for packet counter failed\n");
        return;
    }
    bpf_map_get_next_key(map_fd3,&index,&j);
    bpf_map_lookup_elem(map_fd3, &j, &rec3);
    
    r=tym.tv_nsec+tym.tv_sec*pow(10,9);
    start=r-2*rec3.rx_packets;
    
    flag=0;

    while(1)
    {	    sleep(0.1);
    	    if(bpf_map_get_next_key(map_fd2,&i,&j) != 0)//getting next key for incoming reply packet
    	    {	//Ran out of map. Resetting
    	    	j=-1;
    	    }
    	    clock_gettime(CLOCK_REALTIME, &ts);	
    	    
            if ((bpf_map_lookup_elem(map_fd1, &i, &rec1)) != 0 && flag!=0) {
               i=j;	
               printf("No match...................................\n");
               bpf_map_delete_elem(map_fd2,&i);
               continue;
            }
            
            bpf_map_lookup_elem(map_fd2, &i, &rec2);
            ...
            rTT=abs(rec1.rx_packets-rec2.rx_packets);
	    if(flag!=0)
	    {	printf("RTT: %llu\n",rTT);
	    	bpf_map_update_elem(map_fd4,&i,&rTT,1);
	    }	    
	    flag=1;
